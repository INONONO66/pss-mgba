...
describe("DevViewerServer", () => {
  it("serves the live frame and latest LLM context images for one active run", async () => {
    ...
    try {
      ...
      expect(html).toContain("Input context images + event log");
      ...
      expect(html).toContain("grid-template-columns: minmax(456px, 1.16fr) minmax(360px, 0.92fr) minmax(470px, 1fr)");
