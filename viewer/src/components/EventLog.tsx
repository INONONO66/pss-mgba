...
function rows(event: RunEvent): Array<[string, string]> {
...
export default function EventLog({ payload }: { payload: EventsResponse | null }) {
  ...
  return (
    <article className="panel context-panel">
      ...
      <div className="context-tabs"><button className={`tab ${tab === "events" ? "active" : ""}`} onClick={() => setTab("events")}>이벤트</button><button className={`tab ${tab === "raw" ? "active" : ""}`} onClick={() => setTab("raw")}>원본 로그</button></div>
