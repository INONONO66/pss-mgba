...
export function createMgbaControlPlane({
  client = new MgbaHttpClient({ baseUrl: defaultMgbaBaseUrl() }),
...
function defaultMgbaBaseUrl(): string {
  return process.env.MGBA_HTTP_BASE_URL || "http://127.0.0.1:5000";
}

function defaultRomPath(): string {
  return process.env.MGBA_ROM_PATH || process.env.POKEMON_ROM_PATH || "/tmp/pss-mgba-unconfigured-rom.gb";
}
