export function renderDashboard(): string {
  const lines = [
    ...
    '    .tile-id { color: var(--text-dim); flex: 1; }',
    '    .tile-remove { background: none; border: none; color: var(--text-dim); cursor: pointer; font-size: 14px; padding: 0 2px; line-height: 1; }',
    ...
    '        e.stopPropagation();',
    '        if (!confirm(\'Remove instance #\' + instanceIndex + \'?\')) return;',
    '        if (!adminToken) { alert(\'Admin token required\'); return; }',
    '        var instId = instanceId;',
    '        fetch(\'/admin/instances/\' + instId, {',
    '          method: \'DELETE\',',
    '          headers: { \'X-Admin-Token\': adminToken }',
    '        }).then(function (resp) {',
    '          if (!resp.ok) throw new Error(\'HTTP \' + resp.status);',
    '          removeTile(instanceIndex);',
    '          refreshInstances();',
    '        }).catch(function (err) {',
    ...
    '      grid.on(\'dragEnd\', function () { setTimeout(function () { isDragging = false; }, 50); });',
    '    }',
    '',
